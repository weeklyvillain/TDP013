...
/******************************************************************************************************/
/********************** Defining Passport strategy and serialize/deserialize functions ****************/
/******************************************************************************************************/
                ...
                bcrypt.hash(req.query.newPassword, saltRounds, function(err, hashed_pass){
                    ...
                    dbo.collection("Users").insertOne(userObj, function(err, result) {
                        dbo.collection("Users").findOne({LoginName: req.query.newUsername}, function(err, result){
                            var profileObj={ userId: ObjectId(result._id), LoginName: result.LoginName, 
                                             DisplayName: req.query.newDisplayName, FriendsList: [] }
...
app.get('/save', function(req, res){
    ...
    MongoClient.connect("mongodb://localhost:27017", { useNewUrlParser: true }, function (err, db) {
        var dbo = db.db("tdp013");
        var message_obj = { Message: req.query.message, UserPage: req.query.UserPage, UserPosted: req.query.UserPosted, 
...
/******************************************************************************************************/
/********************************** Blocking unwanted methods *****************************************/
/******************************************************************************************************/

app.post("*", function(req, res){res.status(405).send("Status: 405 Wrong Method - Post");});
app.put("*", function(req, res){res.status(405).send("Status: 405 Wrong Method - Put");});
app.delete("*", function(req, res){res.status(405).send("Status: 405 - Wrong Method - Delete");});
app.all("*", function(req, res){res.status(404).send("Status: 404 Something went wrong!");});
